...
function processYear(year) {
    ...
    Object.keys(months).forEach(function(key){
        var el = months[key];
        sumsAsArray.push({month: key, in: el.in, out: el.out});
    });

    completeYear.months = sumsAsArray;
    completeYear.entries = year;
    //completeYear.yearlySums = categorySumsYearly(year, categories);
    //completeYear.monthlySums = categorySumsMonthly(year, categories);
